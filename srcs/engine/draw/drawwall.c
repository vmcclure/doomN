...
/*   Updated: 2019/06/05 16:23:53 by vmcclure         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void drow_wall(uint32_t *p, t_wall wall, t_tga image, float	*offset, float rotate_x)
{
	...
	{		
		dist2 = pow(pow(dx1, 2) + pow(dy1, 2), 0.5) ;
	...
	else 
	{
		...
		if (offset[1] < 1)
		{
			dist2 = pow(pow(dy1, 2) + pow(dx1, 2), 0.5) ;
			dist1 = (float)dist2 / offset[1] - dist2;
			dir1 = (atan((float)(wall.p[1].y - wall.p[0].y)/(float)(wall.p[1].x - wall.p[0].x)));
			wall.p[1].x = wall.p[1].x + dist1/1/offset[1] * cos(dir1);
			wall.p[1].y = wall.p[1].y + dist1/1/offset[1] * sin(dir1);
			// printf ("x0 %f x0 %f x1 %f x1 %f %f dxn %d dist %f %f\n", wall.p[2].x, wall.p[0].x, wall.p[1].x, wall.p[3].x, offset[1], dx1 , dist2, dist1);
		}
	}
	if (offset[2] < 1 && offset[3] < 1)
	{
		
		dist2 = pow(pow(dx4, 2) + pow(dy4, 2), 0.5) ;
		...
		wall.p[2].x = wall.p[2].x - dist1 * cos(dir4);
		wall.p[2].y = wall.p[2].y - dist1 * sin(dir4);
		wall.p[3].x = wall.p[3].x + dist3 * cos(dir4);
		wall.p[3].y = wall.p[3].y + dist3 * sin(dir4);
	...
	else
	{
		if (offset[2] < 1)
		{
			...
			wall.p[2].x = wall.p[2].x - (float)dist1 * cos(dir4);
			wall.p[2].y = wall.p[2].y - (float)dist1 * sin(dir4);
		}
		if (offset[3] < 1)
		{
			dist2 = pow(pow(dx4, 2) + pow(dy4, 2), 0.5) ;
			...
			dir1 = (atan((wall.p[3].y - wall.p[2].y)/(wall.p[3].x - wall.p[2].x)));
			wall.p[3].x = wall.p[3].x + (float)dist1/1/offset[3] * cos(dir4);
			wall.p[3].y = wall.p[3].y + (float)dist1 /1/offset[3]* sin(dir4);
		}
	}
	...
	// 	m[0] = (((float)dist1 / (offset[1] ))) / (float)(image.width) ;
	// }
	// if (offset[3] < 1)
	// {
		
	// 	kef[1] = 0;//(float)maxdist/offset[1] - (float)maxdist;
	// 	m[1] = ((float)dist2/(offset[3] )) / (float)(image.width);
	// 	// printf("ln %f lv %f pn %f pv %f %d niz %f verh %f max %d min %d %f %f\n", offset[0], offset[2], offset[1], offset[3], start, dist1 , dist2, maxdist,mindist, m[0], m[1]);
	// }
	// if (offset[0] < 1)
	// {
	...
	// 	distn = (float)dist1/ (1.0 - ((1.0 - offset[1]) + (1.0 - offset[0])));
	// 	m[0] = (float)distn / (float)(image.width);
	// 	kef[0] = distn * (1.0 - offset[0]);
	// }
	// if (offset[2] < 1 && offset[3] < 1)
	// {
	// 	distv = (float)dist2/ (1.0 - ((1.0 - offset[3]) + (1.0 - offset[2])));
